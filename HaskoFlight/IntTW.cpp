...
void IntTW::start(uint8_t* commands, uint8_t commandLength, uint8_t commandFlags, void (*successCallback)(void)) {
  ...
  if (haveBus && canContinue((uint8_t*)cmd)) {
...
void IntTW::twiVect(void) {
  ...
  /*-----------------------------*/
    ...
    if (((count & INTTW_COUNT_MASK) == 0) && canContinue((uint8_t*)cmd)) {
      parseAction();
    }
  ...
  /*---------------*/
    ...
    if (twState == TW_MR_SLA_ACK || twState == TW_MR_DATA_ACK) {
      ...
      /* if count==1 check if more reads on the next command */
        ...
        uint8_t* nextCmdPtr;
        if (count & INTTW_COUNT_FLAG_USE_POINTER) {
          nextCmdPtr = (uint8_t*)cmd;
        } else {
          nextCmdPtr = (uint8_t*)cmd + 1;
        }
...
uint8_t IntTW::readByte(uint8_t address, uint8_t regAddr) {
  uint8_t data;
  readBytes(address, regAddr, 1, &data);
  return data;
}

bool IntTW::writeByte(uint8_t address, uint8_t regAddr, uint8_t data) {
  return writeBytes(address, regAddr, 1, &data);
}

int16_t IntTW::readWord(uint8_t address, uint8_t regAddr) {
  uint8_t buff[2];
  readBytes(address, regAddr, 2, buff);
  return ((int16_t)(buff[0] << 8) | (int16_t)buff[1]);
}

bool IntTW::writeWord(uint8_t address, uint8_t regAddr, int16_t data) {
  uint8_t buff[2];
  buff[0] = (uint8_t)(data >> 8);
  buff[1] = (uint8_t)data;
  return writeBytes(address, regAddr, 2, buff);
}

bool IntTW::writeWords(uint8_t address, uint8_t regAddr, uint8_t count, int16_t* data) {
  uint8_t buff[count * 2];
  for (uint8_t i = 0; i < count; i++) {
    buff[i * 2] = (uint8_t)(data[i] >> 8);
    buff[i * 2 + 1] = (uint8_t)data[i];
  }
  return writeBytes(address, regAddr, count * 2, buff);
}

bool IntTW::sendCMD(uint8_t address, uint8_t cmd) {
  return writeBytes(address, cmd, 0, nullptr);
}
